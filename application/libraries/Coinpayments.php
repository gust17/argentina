...
class Coinpayments
{
	...
	public function __construct()
	{
		...
		$this->private_key = SystemInfo('coinpayments_private_key');
		$this->public_key = SystemInfo('coinpayments_public_key');
		$this->ch = null;
	}
	 ...
	 */
	public function CreateTransactionSimple($amount, $currency1, $currency2, $buyer_email = '', $ipn_url = '', $address = '')
	{
		$req = array(
			'amount' => $amount,
			'currency1' => $currency1,
	 ...
	 * @param ipn_url Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty then it will use the default IPN URL in your account.
	 */
	public function CreateWithdrawal($amount, $currency, $currency2, $address, $auto_confirm = FALSE, $ipn_url = '')
	{
		$req = array(
			'amount' => $amount,
			'currency' => $currency,
